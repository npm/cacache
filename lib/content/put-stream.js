...
function putStream (cache, inputStream, opts, _cb) {
  ...
  hasContent(cache, opts.digest, function (err, exists) {
    ...
    ensureTmp(tmpTarget, opts.uid, opts.gid, function (err) {
      if (err) { return cb(err) }
...
function ensureTmp (tmpTarget, uid, gid, cb) {
  mkdirp(tmpTarget, function (err, made) {
    if (err || !made) { return cb(err) }
    // If we made a directory, make sure to fix
