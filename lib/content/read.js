...
function sizeError (expected, found) {
  const err = new Error(Y`Bad data size: expected inserted data to be ${expected} bytes, but got ${found} instead`)
...
function integrityError (sri, path) {
  const err = new Error(Y`Integrity verification failed for ${sri} (${path})`)
  err.code = 'EINTEGRITY'
  err.sri = sri
  err.path = path
  return err
